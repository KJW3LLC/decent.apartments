...
async function generateAndSaveImage(title, tags) {
  ...
  console.log('  🎨 Trying FLUX.2-klein-4b (new fast model)...');
  ...
  if (imageBase64) {
    ...
    console.log('  🔄 FLUX.2-klein-4b failed, falling back to FLUX.1-dev...');
    imageBase64 = await tryGenerateWithModel(
      prompt,
    ...
    if (imageBase64) {
      ...
      console.log('  🔄 FLUX.1-dev failed, falling back to FLUX.1-schnell...');
      imageBase64 = await tryGenerateWithModel(
        prompt,

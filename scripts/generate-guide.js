...
async function fetchAndSaveImage(topic) {
  ...
  // Try FLUX.2-klein-4b first (newer fast model, 4 steps)
  console.log('\n🎨 Trying FLUX.2-klein-4b (new fast model)...');
  ...
  if (imageBase64) {
    ...
    // Fallback to FLUX.1-dev (slower but historically more reliable)
    console.log('\n🔄 FLUX.2-klein-4b failed, falling back to FLUX.1-dev...');
    console.log('   Note: FLUX.1-dev uses 50 steps vs 4 for FLUX.2-klein-4b, so it takes longer');
    imageBase64 = await tryGenerateWithModel(
      prompt,
    ...
    if (imageBase64) {
      ...
      console.log('\n🔄 FLUX.1-dev failed, falling back to FLUX.1-schnell...');
      imageBase64 = await tryGenerateWithModel(
        prompt,
  ...
  const creditInfo = modelUsed === 'klein'
    ? {
